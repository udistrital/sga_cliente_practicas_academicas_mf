...
export class UserService {
  ...
  private findByDocument(DocIdentificacion: any, Usuario: any, Correo: any) {
    return new Promise<boolean>((resolve, reject) => {
      this.anyService
        ...
        .subscribe((res: any) => {
          if (res !== null) {
            this.processUserResponse(res, DocIdentificacion, resolve, reject);
          } else {
            reject(false);
          }
        });
  ...
  private findByUserEmail(UserEmail: string) {
    return new Promise<boolean>((resolve, reject) => {
      this.anyService
        .get(path, "tercero?query=UsuarioWSO2:" + UserEmail)
        .subscribe((res: any) => {
          if (res !== null) {
            this.processUserResponse(res, null, resolve, reject);
          } else {
            reject(false);
          }
        });
