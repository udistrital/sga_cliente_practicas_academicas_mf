...
export class ListPracticasAcademicasComponent {
  ...
  applyFilter(event: Event) {
    ...
    this.dataSource.filterPredicate = (data: any, filter: string) => {
      ...
      const formattedDate = data.FechaRadicacion
        ? this.formatDate(data.FechaRadicacion)
        : "";
      ...
      matchFilter.push(
        data.TipoSolicitud.Nombre.toLowerCase().includes(filter)
      );
  ...
  filterPracticas(event: any) {
    this.InfoPracticasAcademicas = {
      FechaRadicacion: event.data.FechaSolicitud
        ? moment(event.data.FechaSolicitud, "YYYY-MM-DD").format("DD/MM/YYYY")
        : null,
      Id: event.data.Numero ? event.data.Numero : null,
    };
    if (
      this.InfoPracticasAcademicas.Id === null &&
      this.InfoPracticasAcademicas.FechaRadicacion === null
    ) {
      this.InfoPracticasAcademicas = null;
    }
    const endpoint =
      "practicas-academicas?query=EstadoTipoSolicitudId.TipoSolicitud.Id:23&fields=Id,FechaRadicacion,EstadoTipoSolicitudId";
    this.practicasService
      .getPracticas(endpoint, this.InfoPracticasAcademicas, null)
      .subscribe((practicas: any) => {
  ...
  getPracticasAcademicas(param: any, endpoint: any) {
    ...
    if (param === "process") {
      return this.practicasService.getPracticas(endpoint, null, [
        "Radicada",
        "Rectificar",
    ...
    this.sub = this._Activatedroute.paramMap.subscribe((params: any) => {
      ...
      this.autenticationService.getRole().then((rol: Array<String>) => {
        ...
        if (
          ...
          endpoint =
            "practicas-academicas?query=SolicitudId.EstadoTipoSolicitudId.TipoSolicitud.Id:23";
        } else {
          endpoint =
            "practicas-academicas?query=SolicitudId.EstadoTipoSolicitudId.TipoSolicitud.Id:23,TerceroId:" +
            this.userService.getPersonaId();
        }

        this.getPracticasAcademicas(this.process, endpoint).subscribe(
          (practicas: any) => {
            this.dataSource.data = practicas;
            setTimeout(() => {
  ...
  verPractica(element: any) {
    if (
      element.EstadoId.Nombre == "Requiere modificación" &&
      this.process == "process"
    ) {
      this.router.navigateByUrl(
        `/practicas-academicas/nueva-solicitud/${element["Id"]}/${this.processEncript}`
      );
    } else {
      this.router.navigateByUrl(
        `/practicas-academicas/detalle-practica-academica/${element["Id"]}/${this.processEncript}`
      );
    }
  }
