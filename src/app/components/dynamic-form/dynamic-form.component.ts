...
export class DinamicFormComponent implements OnInit, OnChanges {
  ...
  @Input('clean') clean: boolean = false;
  ...
  onKeyUpEvent(event: Event): string {
  ...
  checkFileSignature(): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      this.normalform.campos.forEach(async (d: any, i: any) => {
        if ((d.etiqueta === 'file' || d.etiqueta === 'fileRev') && !d.ocultar) {
          ...
          if (!valid) {
            ...
            //copilot genera un snackbar con el mensaje del popUpManager
            this.snackBar.open(this.translate.instant('GLOBAL.error'), this.translate.instant('ERROR.contenido_archivo_erroneo_mensaje'), { duration: 3000, panelClass: ["error-snackbar"] });
            reject(false)
          }
        }
        if (len-1 === i) {
